...
export function DrawingToolbar({ mode, setMode, color, width, opacity, onDrawingChange, onClear }: Props) {
  return <div className="drawing-toolbar" role="toolbar" aria-label="Herramientas de dibujo">
    ...
    <div className="color-list" aria-label="Color del trazo">{COLORS.map(value => <button key={value} className={color === value ? 'active' : ''} style={{ '--swatch': value, backgroundColor: value } as React.CSSProperties} onClick={() => onDrawingChange({ color: value })} aria-label={`Color ${value}`} aria-pressed={color === value}/>)}</div>
